...
`imu_filter.h/.c` 保留了上板验证过的静止陀螺零偏校准和一阶低通滤波；调用
`IMU_Filter_Init()` 后，将每次成功读取的 `MPU6050_t` 传给 `IMU_Filter_Update()`。
设备需连续静止约 100 个样本完成零偏校准。`imu_tracker.h/.c` 提供去重力后的短时
相对位移积分及静止速度归零，应用层可按需组合使用，默认主循环不自动启动测试界面。
